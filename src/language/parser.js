...
const parser = (() => {
  ...
  const tokenizer = function (path) {
    ...
    const scanRegex = function () {
      ...
      const isClosingSlash = function (position) {
  ...
  const parser = function (source, recover) {
    let node;
    let lexer;
    ...
    const remainingTokens = function () {
      const remaining = [];
      if (node.id !== '(end)') {
    ...
    const advance = function (id, infix) {
      if (id && node.id !== id) {
        let code;
        if (node.id === '(end)') {
    ...
    // left associative
    const infix = function (id, bp, led) {
    ...
    // <expression> <operator> <expression>
    // right associative
    const infixr = function (id, bp, led) {
      ...
      if (node.id !== ']') {
        for (; ;) {
          ...
          if (node.id === '..') {
            // range operator
            const range = {
              type: 'binary', value: '..', position: node.position, lhs: item,
            };
            advance('..');
    ...
    const objectParser = function (left) {
      const a = [];
      if (node.id !== '}') {
        for (; ;) {
    ...
    var tailCallOptimize = function (expr) {
      ...
      if (expr.type === 'function' && !expr.predicate) {
        const thunk = {
          type: 'lambda', thunk: true, arguments: [], position: expr.position,
        };
        thunk.body = expr;
        result = thunk;
    ...
    const pushAncestry = function (result, value) {
      if (typeof value.seekingParent !== 'undefined' || value.type === 'parent') {
        const slots = (typeof value.seekingParent !== 'undefined') ? value.seekingParent : [];
    ...
    const resolveAncestry = function (path) {
      let index = path.steps.length - 1;
      const laststep = path.steps[index];
      const slots = (typeof laststep.seekingParent !== 'undefined') ? laststep.seekingParent : [];
      if (laststep.type === 'parent') {
        slots.push(laststep.slot);
      }
      for (let is = 0; is < slots.length; is++) {
        let slot = slots[is];
      ...
      switch (expr.type) {
        case 'binary':
          switch (expr.value) {
            case '.':
              ...
              if (result.steps.filter((step) => step.keepArray === true).length > 0) {
            ...
            case '{':
              ...
              result.group = {
                lhs: expr.rhs.map((pair) => [processAST(pair[0]), processAST(pair[1])]),
                position: expr.position,
              };
              break;
            case '^':
              ...
              sortStep.terms = expr.rhs.map((terms) => {
        ...
        case 'unary':
          ...
          if (expr.value === '[') {
            ...
            result.expressions = expr.expressions.map((item) => {
              const value = processAST(item);
              pushAncestry(result, value);
              return value;
            });
          } else if (expr.value === '{') {
            // object constructor - process each pair
            result.lhs = expr.lhs.map((pair) => {
              const key = processAST(pair[0]);
              pushAncestry(result, key);
              const value = processAST(pair[1]);
              pushAncestry(result, value);
              return [key, value];
            });
          } else {
        ...
        case 'partial':
          result = {
            type: expr.type, name: expr.name, value: expr.value, position: expr.position,
          };
          result.arguments = expr.arguments.map((arg) => {
            const argAST = processAST(arg);
