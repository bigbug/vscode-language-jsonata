...
import JsonataDebugSession from './debugSession';
...
/*
 * Since here we run the debug adapter as a separate ("external") process,
 * it has no access to VS Code API.
...
if (port > 0) {
  ...
  Net.createServer((socket) => {
    ...
    const session = new JsonataDebugSession(fsAccessor);
