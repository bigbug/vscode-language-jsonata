...
export default class JsonataDebugSession extends LoggingDebugSession {
  ...
  */
  public constructor(fileAccessor: FileAccessor) {
    ...
    this._runtime.on('stopOnEntry', () => {
      this.sendEvent(new StoppedEvent('entry', JsonataDebugSession.threadID));
    });
    this._runtime.on('stopOnStep', () => {
      this.sendEvent(new StoppedEvent('step', JsonataDebugSession.threadID));
    });
    this._runtime.on('stopOnBreakpoint', () => {
      this.sendEvent(new StoppedEvent('breakpoint', JsonataDebugSession.threadID));
    });
    this._runtime.on('stopOnDataBreakpoint', () => {
      this.sendEvent(new StoppedEvent('data breakpoint', JsonataDebugSession.threadID));
    });
    this._runtime.on('stopOnInstructionBreakpoint', () => {
      this.sendEvent(new StoppedEvent('instruction breakpoint', JsonataDebugSession.threadID));
    });
    this._runtime.on('stopOnException', (exception) => {
      if (exception) {
        this.sendEvent(new StoppedEvent(`exception(${exception})`, JsonataDebugSession.threadID));
      } else {
        this.sendEvent(new StoppedEvent('exception', JsonataDebugSession.threadID));
      }
    });
  ...
  */
  ...
  protected async launchRequest(
    ...
    console.log(`program:${args.program} stopOnEntry:${!!args.stopOnEntry} noDebug:${!args.noDebug}`);
  ...
  /* protected breakpointLocationsRequest(
    ...
    if (args.source.path) {
      const bps = this._runtime.getBreakpoints(args.source.path,
  ...
  protected exceptionInfoRequest(
    ...
    console.log('session:exceptionInfo');
    response.body = {
  ...
  protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    ...
    response.body = {
      threads: [
        new Thread(JsonataDebugSession.threadID, 'main thread'),
  ...
  protected stackTraceRequest(
    ...
    console.log('session:stackTrace');
    ...
    const scopes = this._runtime.getScopes();

    response.body = {
      stackFrames:
        scopes.map((scope, idx) => {
          const { line, col } = scope.currentPosition;

          return new StackFrame(idx + 1, scope.name, this.createSource(scope.file), line + 1, col);
        }).reverse(),
      /* stk.frames.map((f, ix) => {
      ...
      // every requested chunk, results in paging
    };
    this.sendResponse(response);
  }
  ...
  protected async evaluateRequest(
    ...
    switch (args.context) {
      case 'repl':
        ...
        if (matches && matches.length === 2) {
          ...
          const bp = new Breakpoint(mbp.verified,
            this.convertDebuggerLineToClient(mbp.line), undefined,
            this.createSource(this._runtime.sourceFile)) as DebugProtocol.Breakpoint;
          bp.id = mbp.id;
    ...
    if (rv) {
      ...
      response.body = {
        result: reply || `evaluate(context: '${args.context}', '${args.expression}')`,
        variablesReference: 0,
      };
    }
  ...
  /* protected disassembleRequest(
    ...
    const instructions = this._runtime.disassemble(baseAddress + offset,
      count).map((instruction) => {
      ...
      switch (typeof v.value) {
        case 'number':
          if (Math.round(v.value) === v.value) {
            ...
            // enable context menu contribution
